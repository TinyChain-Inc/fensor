//! Reproducible filesystem benchmark: run ignored in release with one test thread.

use std::time::Instant;

use fensor::{Tensor, TensorMatMul, TensorRead, TensorTransform, TensorUnary};
...
async fn measure<V: TensorRead<DType = f32>>(name: &str, view: &V) {
    ...
    let (_, dir) = common::new_dir("matrix_benchmark_copy").await;
    counters::reset_traffic();
    let start = Instant::now();
    let copy: Tensor<FsEntry, f32> = Tensor::copy_from(dir, view, 128).await.unwrap();
...
#[tokio::test]
#[ignore = "release benchmark, not a timing gate"]
async fn matrix_benchmark() {
